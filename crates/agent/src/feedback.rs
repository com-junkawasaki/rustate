    ...
    E: EventTrait + Clone + Debug + Send + Sync + DeserializeOwned + 'static,
{
    ...
    async fn provide_feedback(
        &self,
        decision: &Decision<E>,
    ) -> std::result::Result<Feedback<E>, AgentError>;
}
...
mod tests {
    ...
    fn test_feedback_creation() {
        let feedback =
            Feedback::<TestEvent>::new("優れた決定でした", FeedbackType::Positive, "user");
    ...
    fn test_feedback_with_metadata() {
        let feedback =
            Feedback::<TestEvent>::new("改善の余地があります", FeedbackType::Negative, "user")
                .with_metadata("reviewer", "system")
                .with_metadata("category", "efficiency");

        assert!(feedback.is_negative());
        assert_eq!(
            feedback.metadata.get("reviewer"),
            Some(&"system".to_string())
        );
        assert_eq!(
            feedback.metadata.get("category"),
