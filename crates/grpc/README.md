...
`rustate-grpc`は`rustate`コアライブラリの機能をネットワーク越しに利用可能にする拡張クレートです：

- **単一責任の原則**: gRPCとネットワーク機能のみに集中し、コアステートマシン機能は`rustate`に任せています
- **最小限の依存関係**: ローカルでのみステートマシンを使用する場合は`rustate`だけで十分です
- **シームレスな統合**: `rustate`のモデルとgRPCサービス間で型安全な変換を提供

`rustate`と`rustate-grpc`を組み合わせることで、分散システムやマイクロサービスアーキテクチャでも型安全なステート管理を実現できます。
...
利用シナリオに応じて機能を選択できます：
- `client` - クライアント機能のみ必要な場合
- `server` - サーバー機能のみ必要な場合
- `full` - クライアントとサーバー両方の機能が必要な場合（デフォルト）
